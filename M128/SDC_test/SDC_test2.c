  ...
  ASA_SDC00_put(ASA_ID, 64, 8, "SDC8");
	...
	for(int i=0; i<4000; i++) {
      ...
      if(i%4==1)
      {
        ...
        if(position[i-1]>=65 && position[i-1]<=70)
        {position[i-1]=position[i-1]-55;}

        if(position[i]>=48 && position[i]<=57)
        {position[i]=position[i]-48;}
        if(position[i]>=97 && position[i]<=102)
        {position[i]=position[i]-87;}
        if(position[i]>=65 && position[i]<=70)
        {position[i]=position[i]-55;}
