...
int main(void)
{
	...
	while(1)
  {
      ...
      if(i==2)   //i==2表示已經收集完兩筆資料
      {
			//ID=get[0];
			printf("get ID=%d\n",get[0] );       //收到的ID
			printf("get position=%d\n",get[1] );//收到的position
			ID_convert(get[0]);    //轉換ID
			convert_1(get[1]);     //轉換position
			//printf("get[0]=%d,get[1]=%d\n",get[0],get[1] );
			printf("converted ID:%d ,converted position:%d ,side=%d\n",ID,data_1,side);//轉換過後的ID，position，side(左或右半邊)

			//USART0_Transmit(128+ID);       //傳送ID
			//_delay_ms(100);
			//USART0_Transmit(data_1>>7);  //傳送角度
			//_delay_ms(100);
			//USART0_Transmit(data_1&127);  //傳送角度
      ...
      for(int j=0;j<100;j++)
		...
		if(mode==129)         //mode2 手動教學模式
		{
			printf("start mode%d\n",mode);
			printf("mode129 cannot use now\n" );
			// for(k=0;k<17;k++)
			// {
